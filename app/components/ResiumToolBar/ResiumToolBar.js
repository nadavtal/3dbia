...
const ResiumToolBar = (props) => {
  ...
  const updatedActionsMultipliers = (actionGroups) => {
    ...
    actionGroups.calibration.actions[5].defaultValue = incomingCalibrationData['rotate-x'];
    actionGroups.calibration.actions[6].value = 0;
    actionGroups.calibration.actions[7].defaultValue = incomingCalibrationData['rotate-y'];
    actionGroups.calibration.actions[8].value = 0;
    actionGroups.calibration.actions[9].defaultValue = incomingCalibrationData['rotate-z'];
...
/>)
  ...
  const actionsHtml = actionGroups[actionGroup.toLowerCase()].actions.map((action, index) => {
    ...
    switch (action.elementType) {
      case 'text':
        return <h6 key={index} className="bold pt-2">{action.label}</h6>
      case 'button':
        return <MDBBtn
            ...
            <div className={"resiumToolBar_actionsWrapper"}>
              ...
              {showButtons && <>
                ...
                <MDBBtn color="primary" size="sm"
                  ...
                  disabled={!props.selectedSubTask ||( props.selectedSubTask && props.selectedSubTask.name !== 'Calibrate models')}
                  >Save</MDBBtn>
                <MDBBtn color="danger" size="sm" className="saveViewButton"
                  onClick={() => props.saveView()}>Save View</MDBBtn>
