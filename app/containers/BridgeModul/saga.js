...
function* getFolderStructure(action) {
  try {
    console.log(action);
    // Call our request helper (see 'utils/request')
    const folder_structure = yield call(
      request,
      `${apiUrl}folder_structure/${action.organization_id}`,
    );
...
export const getFilesImportantData = files => {
  ...
  const images = files.smallImages.map(file => {
    ...
    const fullImage = files.fullImages.find(fullImagefile =>
      file.name.includes(fullImagefile.name.split('.')[0]),
    );
...
export default function* projectSaga() {
  ...
  yield takeLatest(
    actionTypes.UPDATE_BRIDGE_PRIMARY_MODEL_ID,
    updateBridgePrimaryModelId,
  );
  yield takeLatest(actionTypes.UPDATE_SURVEY_STATUS, updateSurveyStatus);
  yield takeLatest(
    actionTypes.UPDATE_BRIDGE_DEFAULT_VIEW,
