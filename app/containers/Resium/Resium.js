...
const Resium = props => {
  ...
  const [selectedItem, setSelectedItem] = useState(
    props.models.length && props.models[0] !== undefined ? props.models[0].name : '',
  );
  ...
  useEffect(() => {
    if (props.bridge) {
      let bridgeLocation = Cesium.Cartesian3.fromDegrees(props.bridge.lon, props.bridge.lat)
  ...
  useEffect(() => {
    ...
    setPointsPrimitives([])
    if (props.models.length && !hasTileSet(props.models) && !props.bridge.default_view_data && viewerRef.current) {
      zoomToBridge()
    }
  ...
  const zoomToBridge = () => {
    let bridgeLocation = Cesium.Cartesian3.fromDegrees(props.bridge.lon, props.bridge.lat, 300)
  ...
  const setModelsControlerStates = models => {
    ...
    models.map((model, index) => {
      ...
      let center = null;
      if (model.calibration_data && model.calibration_data !== 'undefined' && model.calibration_data !== 'null') {
        ...
        calibrationData = {
          ...
          height: props.bridge && props.bridge.height? props.bridge.height : 100,
          'rotate-x': 0,
          'rotate-y': 0,
          'rotate-z': 0,
        };
      ...
      let show = false
      if ((model.id && model.id == props.bridge.primary_model_id) || model.type == 'model' || model.isNew) show = true
  ...
  const handleViewerRightClick = (m, t) => {
    ...
    if (props.selectedSubTask && props.selectedSubTask.name == 'Calibrate models') {
      setOverLayPosition({
      ...
      setRightClickMenuActions([
        {name: 'Position calibration globe here'}
      ])
  ...
  const handleViewerMouseMove = (movement, t) => {
    ...
    if (calibrationMode == 'lon') {
      ...
      const height = viewerRef.current.cesiumElement.camera.positionCartographic.height.toFixed(2)
        ...
        if (!Cesium.defined(pickedFeature)) {
          ...
          if (pickedFeature.node && pickedFeature.node.name) {
            ...
            if (pickedFeature.node.name !== overLayName)
              setOverLayName(hoveredElement ? hoveredElement.name : pickedFeature.node.name);
  ...
  const addLineEntity = (positions) => {
    return viewerRef.current.cesiumElement.entities.add({
      polyline: {
        positions: positions,
        followSurface: false,
        width: 3,
      ...
      if (scene.pickPositionSupported && Cesium.defined(pickedFeature)) {
        if (Cesium.defined(cartesian)) {          
          ...
          if (pointsCollection.length === 0) {
            ...
            const distanceLine = addLineEntity([pointsPrimitives[0].position, point2.position])
  ...
  const models = useMemo(() => props.models.map(model => {
  ...
  const tiles = useMemo(() => props.models.map(model => {
    ...
    >
      ...
      {calibrationState && (
        <>
          ...
          {movingPolylinePositions && viewerRef.current && 
          ...
          {closingPolylinePositions && props.mode === 'Measure polygone' && viewerRef.current && (
            <Polyline
              flyTo={false}
              center={Cesium.Cartesian3.fromDegrees(34.91307, 32.41295)}
              viewer={viewerRef.current.cesiumElement}
...
export function mapDispatchToProps(dispatch) {
  return {
    ...
    onNodeSelected: (nodeName, selectMultiple) => dispatch(actions.elementSelected(nodeName, selectMultiple)),
    onFocusElement: (nodeName) => dispatch(setFocusedElement(nodeName)),
    onRightMenuOptionClick: (action, data) => dispatch(actions.onRightMenuOptionClick(action, data)),
    editElement: element => dispatch(editElement(element)),
    updateElements: element => dispatch(updateElements(element)),
    onModelLoaded: (nodes, model) => dispatch(actions.modelLoaded(nodes, model)),
    save: (model) => dispatch(updateModel(model)),
    onUpdateBridgeDefaultView: (view, bid) => dispatch(updateBridgeDefaultView(view, bid)),
    onElementsSelected: (ids, selectSingle) => dispatch(actions.elementsSelected(ids, selectSingle)),
