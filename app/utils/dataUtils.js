...
export const getProviderById = (provId, providers) =>
  providers.find(prov => prov.id === provId);
...
export const getOrganizationUsersByProvider = (provider_id, orgUsers) =>
  orgUsers.filter(orgUser => orgUser.from_provider_id === provider_id);

export const getAllUsersByProvider = (providersUsers, provider) =>
  providersUsers.filter(connection => connection.provider_id === provider.id);
...
export const getAllProvidersUsers = (providers, providersUsers, users) => {
  ...
  const providersUsersObject = {};
  providers.map(provider => {
    // console.log(provider)
    providersUsersObject[provider.name] = [];
    const providersUsersConnections = getAllUsersByProvider(
      providersUsers,
      provider,
    );
    providersUsersConnections.map(userConnection =>
      providersUsersObject[provider.name].push(
        getUserById(userConnection.user_id, users),
      ),
    );
  });
...
export const addProviderToRoles = (roles, providers) => {
  ...
  roles.forEach(role => {
    if (role.provider_id)
      role.provider = providers.find(prov => prov.id == role.provider_id).name;
...
export const getAvailableRolesByUserId = (allRoles, accupiedRoles) => {
  ...
  const availableRoles = allRoles.filter(role => !accupiedRoles.includes(role));
  return availableRoles;
};
export const getAvailableUsersByRoleId = (roleId, userRoles) => {
  // console.log(allRoles, accupiedRoles)
  const availableRoles = userRoles.filter(role => role.role_id !== roleId);
  return availableRoles;
};
...
export const getUniqueValuesFromColumn = (field, data) => {
  const uniqueValues = [];
  data.map(row => {
    if (row[field] !== '' && !uniqueValues.includes(row[field]))
...
export const getLatestUpdateditem = items =>
  ...
  items.sort((a, b) => b.updatedAt - a.updatedAt)[0];
...
export const getUniqueUsers = array => {
  ...
  const uniqueUsers = uniqueIds.map(id =>
    array.find(item => item.user_id == id),
  );
  return uniqueUsers;
};
...
export const getFileNameByString = (str, files) =>
  files.filter(file => file.name.includes(str));

export const splitStringToArray = (str, charecter) => str.split(charecter);
