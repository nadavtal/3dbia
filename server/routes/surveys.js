...
app.get('/surveys/:id', function(req, res) {
  ...
  let q = 'SELECT * FROM tbl_survey WHERE id = ' + req.params.id;
  connection.query(q, function(error, results) {
...
app.get("/surveys/:id/models", function(req, res){
  ...
  let q = 'SELECT * FROM tbl_bridge_models WHERE survey_id = ' + req.params.id;
  connection.query(q, function(error, results) {
    if (error) throw error;
    // console.log(results)
    res.send(results);
...
const getTiles = publicUrls => 
  ...
  //   })
  // })
   Promise.all(publicUrls.map(url => {
    console.log(url)
...
const createTiles = urls => Promise.all(urls.map(url => {
    return new Promise((resolve, reject) => {
...
const checkIfTileExistAndAddIfNot = async (url, bid, surveyId) => new Promise((resolve, reject) => {
...
const checkIfTilesExistAndAddIfNot = async (publicUrls, bid, surveyId) => 
   Promise.all(publicUrls.map(url => checkIfTileExistAndAddIfNot(url, bid, surveyId)))
...
async function createSurveyTasksAndFoldersInGoogleCloud(
  survey,
  templateTasks,
  res,
) {
...
async function createSurveyTasksAndFoldersInGoogleDrive(survey) {
  const destinationBridgeFolder = await drive.files.list({
  ...
  const bridgeFolderChildren = await drive.files.list({
    // q: `name = "bid_${survey.bid}"`,
  ...
  if (surveyResult.insertId) {
    ...
    // console.log("newTasksResult", newTasksResult)
    // create folders
    const newDriveFolderResult = await createGoogleDriveFolders(
      survey,
      testinationSurveysFolderId,
    );
...
const createSurvey = (survey) => new Promise((resolve, reject) => {
...
const createFolderAndSubFoldersInGCS = async (bucketName, mainFolder, subFolders) => {
  try {
    const mainSurveyFolderResult = await storage
      .bucket(bucketName)
    ...
    console.log("mainSurveyFolderResult", mainSurveyFolderResult)
    try {
      const subFoldersResult = await Promise.all(subFolders.map(subFolder => storage.bucket(bucketName).upload("base_folder_file.txt", {
...
const createFolder = (folderName, parentId) => new Promise((resolve, reject) => {
      const fileMetadata = {
        "name": folderName,
...
const addFoldersToDb = (folders) => new Promise((resolve, reject) => {
...
const addFolderToDb = (survey_id, bid, task_id, data_type, folder_url, folder_id) => new Promise((resolve, reject) => {
      const q = `INSERT INTO tbl_survey_data (
        bid,
        task_id,
        survey_id,
