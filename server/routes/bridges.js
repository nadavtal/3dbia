...
app.post('/bridges', upload.single('file'), function(req, res) {
  ...
  createBridgeAndFolder(bridge, bridgesFolderId, ['Surveys', 'General']).then(
...
const createBridgeAndFolder = async (bridge, parentId, subFolders) => {
  try {
    ...
    const folder = await storage
      .bucket(bucketName)
      .upload('base_folder_file.txt', {
        destination: `bid_${bridgeResult.insertId}/` + 'base_folder_file.txt',
      });
    console.log('folder', folder);
    const subFolder = await storage
      .bucket(bucketName)
      .upload('base_folder_file.txt', {
        destination:
          `bid_${bridgeResult.insertId}/` +
          'general_files/base_folder_file.txt',
      });
...
const createNewBridge = bridge => new Promise((resolve, reject) => {
...
const createFolderInGoogleDrive = (folderName, parentId) => new Promise((resolve, reject) => {
    //google drive api
    const fileMetadata = {
      "name": folderName,
...
const createFolders = (folderNames, parentId) => Promise.all(folderNames.map(async (folderName) => {
...
const createBridgeFolders = (parentId, folders) => new Promise((resolve, reject) => {
...
app.get('/bridges/:id', async function(req, res) {
  ...
  try {
    ...
    const bucketName = `3dbia_organization_${
      basicBridgeInfo[0].organization_id
    }`;
...
const getBridgeBasicInfo = bridgeId => new Promise((resolve, reject) => {
...
const getBridgeMainGlbModel = modelId => {
  ...
  return new Promise((reject, resolve) => {
    let q = 'SELECT * FROM tbl_bridge_models WHERE id = ' + modelId;
    // console.log(q)
    connection.query(q, function(error, results) {
      if (error) {
        reject(error);
...
const getBridgeDetails = bridgeId => {
  return new Promise((reject, resolve) => {
    let q = 'SELECT * FROM tbl_custom_fields WHERE parent_id = ' + bridgeId;
    console.log(q);
    connection.query(q, function(error, results) {
      if (error) {
        reject(error);
...
function updateBridgeField(field, bridgeId, res) {
  console.log(bridgeId);
  let q = `UPDATE tbl_custom_fields
...
app.get('/bridges/:id/processes', function(req, res) {
  ...
  let q = 'SELECT * FROM tbl_processes WHERE bid = ' + req.params.id;
  connection.query(q, function(error, results) {
...
app.get('/bridges/:id/tasks', function(req, res) {
  ...
  const {id} = req.params
  console.log('getting bridge processes');
  let q = 'SELECT * FROM tbl_tasks WHERE bid = ' + req.params.id;
  connection.query(q, function(error, results) {
    if (error) throw error;
...
app.get('/bridges/:id/spans', function(req, res) {
  ...
  console.log(req.params);
  const {id} = req.params
  let q = 'SELECT * FROM tbl_bridge_spans WHERE bid = ' + req.params.id;
  console.log(q);
  connection.query(q, function(error, results) {
    if (error) throw error;
...
app.delete('/spans/:spanId/:userId', async function(req, res) {
  ...
  connection.query(resetElementQ, function(err, result) {
    ...
    // return(result);
    let q = 'DELETE FROM tbl_bridge_spans WHERE id = ' + req.params.spanId;
    console.log(q);
    connection.query(q, function(error, results) {
      if (error) throw error;
...
const createSpans = async spans => Promise.all(spans.map(span => createSpan(span)))

const createSpan = span => new Promise((resolve, reject) => {
...
const updateSpans = async spans => Promise.all(spans.map(span => updateSpan(span)))

const updateSpan = span => new Promise((resolve, reject) => {
...
app.get('/bridges/:id/elements', function(req, res) {
  ...
  let q = `SELECT be.*, u.first_name as updatedBy FROM tbl_bridge_elements be
...
app.post('/bridges/:id/elements', function(req, res) {
  ...
  let q = `INSERT INTO tbl_bridge_elements (bid, span_id, bridge_model_id, object_id, name, updated_by_user_id, last_updated) VALUES ?`;
...
const updateElementQuery = element => {
  ...
  SET span_id = ${element.span_id},
      ...
      secondary_quantity = ${
        element.secondary_quantity ? element.secondary_quantity : null
      },
...
app.post('/bridges-elements/createUpdateDelete/:userId', async function(
  req,
  res,
) {
  ...
  // let resetedElements = []
  try {
    const deletedElements = req.body.deleteElementsIds
      ? await elementsController.deleteElements(req.body.deleteElementsIds)
      : [];
    const newElements = req.body.newElements
      ? await elementsController.createElements(req.body.newElements)
      : [];
    const resetedElements = req.body.resetElemetsIds
      ? await elementsController.resetElements(
          req.body.resetElemetsIds,
          req.params.userId,
        )
      : [];
...
app.put('/bridges-spans/:id', function(req, res) {
  ...
  const span = req.body;
  let q = `UPDATE tbl_bridge_spans
  SET name = "${span.name}",
      structure_type_id = ${
        span.structure_type_id ? span.structure_type_id : null
      },
      description = "${span.description}",
      span_order = ${span.span_order},
      span_area = ${span.span_area ? span.span_area : null},
...
app.get('/bridges-models/:id', function(req, res) {
  ...
  let q = 'SELECT * FROM tbl_bridge_models WHERE bid = ' + req.params.id;
  connection.query(q, function(error, results) {
    if (error) throw error;
...
app.post('/bridges-models', async function(req, res) {
  console.log('creating new bridge model', req.body);
  const model = req.body;
  // const values = createProjectsArray([bridge])[0]
  // console.log(values)
  const result = await modelsController.createBridgeModel(model);
  res.send(result);
...
app.put('/bridges-models/:id', function(req, res) {
  ...
  const model = req.body;
  let q = `UPDATE tbl_bridge_models
...
app.post('/bridges-models/delete/:id', async function(req, res) {
  ...
  let q = `DELETE FROM tbl_bridge_models WHERE id=${req.params.id}`;
